 ...
 */
static long long random_val(long long min, long long max)
{
    double weight = rand()/(double) RAND_MAX;
    long long result = min * (1-weight) + max * weight;
    return result;
}
 ...
 */
static long long gen_vals(long long test_vals[], long long min, long long max, long long test_range, long long arg)
{
    long long i;
	...
	if ((min == 1 && max == 1)) {
	unsigned long long smallest_norm = 0x100000;
	smallest_norm <<= 32;
	unsigned long long one = 0x3ff00000;
	one <<= 32;
	unsigned long long largest_norm = 0x7ffeffff;
	largest_norm = (largest_norm << 32) | 0xffffffff;
	
	unsigned long long inf = 0x7ff00000;
	inf <<= 32;
	unsigned long long nan = 0x7ff80000;
	nan <<= 32;
	unsigned long long sign = 0x80000000;
	sign <<= 32;

	/* Test range should be at most 1/2 the range of one exponent
	   value */
	if (test_range > (long long)1 << 52) {
	    test_range = (long long)1 << 52;
	}
	// =========== my code ends ================
 ...
 */
static long long test_0_arg(funct_t f, funct_t ft, char *name)
{
    long long r = f();
    long long rt = ft();
    long long error =  (r != rt);

    if (error && !grade)
	printf("ERROR: Test %s() failed...\n...Gives %lld[0x%llx]. Should be %lld[0x%llx]\n", name, r, r, rt, rt);
 ...
 */
static long long test_1_arg(funct_t f, funct_t ft, long long arg1, char *name)
{
    funct1_t f1 = (funct1_t) f;
    funct1_t f1t = (funct1_t) ft;
    long long r, rt, error;

    r = f1(arg1);
    rt = f1t(arg1);
    error = (r != rt);
    if (error && !grade)
	printf("ERROR: Test %s(%lld[0x%llx]) failed...\n...Gives %lld[0x%llx]. Should be %lld[0x%llx]\n", name, arg1, arg1, r, r, rt, rt);
 ...
 */
static long long test_2_arg(funct_t f, funct_t ft, long long arg1, long long arg2, char *name)
{
    funct2_t f2 = (funct2_t) f;
    funct2_t f2t = (funct2_t) ft;
    long long r = f2(arg1, arg2);
    long long rt = f2t(arg1, arg2);
    long long error = (r != rt);

    if (error && !grade)
	printf("ERROR: Test %s(%lld[0x%llx],%lld[0x%llx]) failed...\n...Gives %lld[0x%llx]. Should be %lld[0x%llx]\n", name, arg1, arg1, arg2, arg2, r, r, rt, rt);
 ...
 */
static long long test_3_arg(funct_t f, funct_t ft, 
		      long long arg1, long long arg2, long long arg3, char *name)
{
    funct3_t f3 = (funct3_t) f;
    funct3_t f3t = (funct3_t) ft;
    long long r = f3(arg1, arg2, arg3);
    long long rt = f3t(arg1, arg2, arg3);
    long long error = (r != rt);

    if (error && !grade)
	printf("ERROR: Test %s(%lld[0x%llx],%lld[0x%llx],%lld[0x%llx]) failed...\n...Gives %lld[0x%llx]. Should be %lld[0x%llx]\n", name, arg1, arg1, arg2, arg2, arg3, arg3, r, r, rt, rt);
 ...
 */
static long long test_function(test_ptr t) {
    long long test_counts[3];    /* number of test values for each arg */
    long long args = t->args;    /* number of function arguments */
    long long arg_test_range[3]; /* test range for each argument */
    long long i, a1, a2, a3;        
    long long errors = 0;

    /* These are the test values for each arg. Declared with the
       static attribute so that the array will be allocated in bss
       rather than the stack */
    static long long arg_test_vals[3][MAX_TEST_VALS]; 

    /* Sanity check on the number of args */
    if (args < 0 || args > 3) {
	printf("Configuration error: invalid number of args (%lld) for function %s\n", args, t->name);
	exit(1);
    }
	...
	if (rc) {
	    ...
	    errors = 1;
	    printf("ERROR: Test %s failed.\n  Timed out after %lld secs (probably infinite loop)\n", t->name, timeout_limit);
	    return errors;
	}
	alarm(timeout_limit);
    }
...
static long long run_tests() 
{
	...
	if (!test_fname || strcmp(test_set[i].name,test_fname) == 0) {
	    long long rating = global_rating ? global_rating : test_set[i].rating;
